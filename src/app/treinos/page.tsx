        ...
        <TrainingSelector
          onConfirm={(s) => {
            ...
            try {
              ...
              setGeneratedWorkouts(Array.isArray(plan) ? plan : []);
              setHighlightIndex(0);
            } catch (err) {
              console.error('Erro ao gerar plano automático de treino:', err);
                  ...
                  >
                    ...
                    {Array.isArray(workout.blocks) && workout.blocks.length > 0 && (
                      <ul className="space-y-1 text-[11px] text-slate-400">
                        {workout.blocks.slice(0, 3).map((block: any, i: number) => (
                          <li key={i}>
                            <span className="font-semibold text-slate-300">
                              {block.label ?? block.name ?? `Bloco ${i + 1}`}:
                            </span>{' '}
                            {block.description ??
                              block.details ??
                              block.duration ??
